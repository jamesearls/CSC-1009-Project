...
import views.DonatepingendoView;
import views.AddTopicPageView;
import views.AppointmentListView;
import views.AddTopicPageView;
import views.ExampleView;
import views.HelpView;
import views.HomeView;
import views.MainForumPageView;
import views.SignUpView;
import views.SymptomCheckerView;
import views.TestForm;
import views.YourAccountPageView;
import web.Base64;
...
public class Main 
{
	...
	{		
		...
		DonatepingendoView DonatepingendoviewPage = new DonatepingendoView(databaseInterface, fileStoreInterface);
		...
		MainForumPageView mainForum = new MainForumPageView(databaseInterface,fileStoreInterface);
		YourAccountPageView accountPage = new YourAccountPageView(databaseInterface,fileStoreInterface);
		AddTopicPageView addTopic = new AddTopicPageView(databaseInterface,fileStoreInterface);
        HomeView homePage = new HomeView(databaseInterface, fileStoreInterface);
		SymptomCheckerView symptomChecker = new SymptomCheckerView(databaseInterface,fileStoreInterface);
		AppointmentListView appointmentListView = new AppointmentListView(databaseInterface,fileStoreInterface);
		AccountListView	accountListView = new AccountListView(databaseInterface,fileStoreInterface);


//		AccountListView accountListview = new AccountListView(databaseInterface,fileStoreInterface);

		//ths variable indicates that the program should keep running
		...
		while(shouldKeepRunning)
		{
			...
			for (int i = 0; i < requestToProcess.size(); i++) 
			{
		        ...
		        else if(DonatepingendoviewPage.process(toProcess)) {
		        	
		        }else if(symptomChecker.process(toProcess)) {
		        ...
		        }else if(mainForum.process(toProcess)) {
		        	
		        }else if(accountPage.process(toProcess)) {
		        	
		        }else if(addTopic.process(toProcess)) {
		        	
		        }
		        else if(homePage.process(toProcess)) {
		        	
		        }
		        else if(appointmentListView.process(toProcess)) {
		        	
		        }
		        else if(accountListView.process(toProcess)) {
